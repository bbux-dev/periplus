...
Life Log is built bottom-up by dependency: first a runnable, installable, offline-capable PWA shell with the locked architecture in place; then the local data layer (Dexie + `LifeLogEntry` + repository) that everything persists through; then the navigation skeleton (dashboard → category → entry type) that gives the app its spine. With those in place, the product's signature URL-first capture flow lands, followed by the secondary manual-entry path, and finally the read side — entry list, detail, and JSON export. Each phase delivers a coherent, verifiable capability and unblocks the next.
...
- [ ] **Phase 1: Foundation & PWA Shell** - Scaffold the locked stack into an installable, offline-capable app shell
- [ ] **Phase 2: Data Layer** - Dexie database, `LifeLogEntry`, and a local repository with reactive reads
...
**Goal**: A mobile-first, installable PWA shell runs locally and survives going offline, with the locked architecture scaffolded.
**Depends on**: Nothing (first phase)
**Requirements**: SETUP-01, SETUP-02, SETUP-03, SETUP-04, PWA-01, PWA-02, PWA-03, PWA-04
**Success Criteria** (what must be TRUE):
  1. Developer runs the dev server and the app loads a home shell on a phone-sized viewport
  2. The app can be installed to a phone home screen (web manifest + registered service worker)
  3. After first load, the app shell opens while the browser/device is offline
  4. `tsc -b && vite build` succeeds with the template directory layout (`pages`, `components/ui`, `services`, `state/common`, `config`, `pwa`, `assets`) and `cn`/`Button`/`Input`/`FormField` primitives present
**Plans**: TBD
**UI hint**: yes

### Phase 2: Data Layer
**Goal**: `LifeLogEntry` records persist locally in IndexedDB through a repository, and reads are reactive.
**Depends on**: Phase 1
**Requirements**: DATA-01, DATA-02, DATA-03, DATA-04, DATA-05
**Success Criteria** (what must be TRUE):
  1. A `LifeLogEntry` can be written and read back from IndexedDB via `entriesRepository`
  2. Stored entries survive a page refresh (persisted, not in-memory)
  3. A component using `useLiveQuery` re-renders automatically when entries change
  4. An "unsynced entries" query returns local entries, proving the future-sync seam exists
**Plans**: TBD
...
**Requirements**: CAPT-01, CAPT-02, CAPT-03, CAPT-04, CAPT-05, CAPT-06, PWA-05
**Success Criteria** (what must be TRUE):
  ...
  5. An entry can be created and persisted while offline
**Plans**: TBD
**UI hint**: yes
...
|-------|----------------|--------|-----------|
| 1. Foundation & PWA Shell | 0/TBD | Not started | - |
