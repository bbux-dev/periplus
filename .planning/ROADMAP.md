...
- ✅ **v0.5.0 — Trips MVP UI Refactor** — Phases 20–24 (shipped 2026-06-19)


## Phases

<details>
<summary>✅ v0.5.0 — Trips MVP UI Refactor (Phases 20–24) — SHIPPED 2026-06-19</summary>

Aggressively rewrote the UI to expose ONLY a minimal, mobile-first trip logger — create/activate a
trip, log expenses + activities tied to the active trip, view prior trips, and see per-trip
category-grouped expense reports — while preserving the headless engine (Dexie, `entriesRepository`,
active-mode/context stamping, `draftToEntry`). All 13 prior screens + the DSL/shortcut/layout
subsystem were dropped; ui primitives + the engine were reused. North star: `seeds/fewest-buttons-slickest.md`.

- [x] **Phase 20: Trip Data Model + Engine Extensions** — `EntryType` += `trip`/`activity`;
  `ActiveMode.tripId` + backward-compatible `activateMode`; `draftToEntry` stamps `metadata.tripId`;
  new `tripService` (create-and-activate, list, pure stat helpers, reactive hooks). Reqs: ENG-01..04. (2 plans)
- [x] **Phase 21: App Shell + Routing Rewrite + Atomic Drop** — atomic deletion of 11 non-trip pages +
  the DSL/shortcut/layout subsystem (51 files, suite stayed green); trip-only router; reworked
  `AppShell`; export-only Settings; `CreateTripPage` + `TripHomePage` stub (loading-vs-no-trip guard).
  Reqs: UI-01..05, TRIP-02, TRIP-04. (4 plans)
- [x] **Phase 22: Trip Home + Expense Capture** — full `TripHomePage` (name, formatUSD total, recent
  entries, Expense + Activity CTAs); `ExpenseSheet` (amount → 8-category grid → save, `domain:'trips'`,
  local-date, tripId stamp); `formatUSD`; `EXPENSE_CATEGORIES`. Reqs: TRIP-01/03, HOME-01..05, EXP-01..06. (3 plans)
- [x] **Phase 23: Activity Capture** — `ActivityTypePage` (Hike/Show/Restaurant/Cafe/Other);
  `ActivityFormPage` (Name req; Location/Rating/Notes; Other adds required Type); accessible 1–5
  `StarRating`. Reqs: ACT-01..06. (3 plans)
- [x] **Phase 24: Previous Trips + Trip Detail + Expense Report** — `PreviousTripsPage` (single-pass
  stats) → `TripDetailPage` (drill-in by UUID); category-grouped float-safe `ExpenseReport` +
  chronological timeline + merge-preserving inline edit/delete (reactive). Reqs: PREV-01..04, RPT-01..06. (3 plans)

Full phase details archived in [`milestones/v0.5.0-ROADMAP.md`](milestones/v0.5.0-ROADMAP.md).
Audit: [`milestones/v0.5.0-MILESTONE-AUDIT.md`](milestones/v0.5.0-MILESTONE-AUDIT.md).
...
| 24. Previous Trips + Trip Detail + Expense Report | v0.5.0 | 3/3 | Complete    | 2026-06-19 |
